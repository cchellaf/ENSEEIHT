...
%liste_personnes_base = {'f01', 'f02', 'f10', 'm01', 'm02', 'm08'}
%       personnes          1     2      10     17     18     24     
...
somme_vq = sum(D(1:q)); 
somme_vp = sum(D); 
Contraste_sans_masque = somme_vq/somme_vp
...
% SUBSPACE ITERATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%[V_2, D] = eig(Sigma2_masque);
[ D_masque, W_masque, it, flag ] = subspace_iter_v0( Sigma2_masque, n, 1e-8, 10000 );

% Les vecteurs propres de Sigma (les eigenfaces) se deduisent de ceux de Sigma2 :
%V_masque = transpose(X_centre_masque)*V_2;
W_masque = transpose(X_centre_masque)*W_masque; 
...
%Calcul des contrastes 
somme_vq = sum(D_masque(1:q)); 
somme_vp = sum(D_masque); 
Contraste_avec_masque = somme_vq/somme_vp
