...
type typ = Bool | Int | Rat | Undefined | Pointeur of typ | TNomme of string | Struct of (typ * string) list 
...
let rec string_of_type t = 
  ...
  | TNomme s -> "TNomme " ^ s
  | Struct l -> "Struct {  " ^ String.concat "\n " (List.map (fun (t,s) -> (string_of_type t) ^ s ) l) ^ "}"
...
let getTaille t =
  ...
  | Undefined -> 0
  | Pointeur _ -> 1 
  | _ -> 1 (** on redefini cette fonction après le passe de typage pour prendre en compte les types nommes *)
