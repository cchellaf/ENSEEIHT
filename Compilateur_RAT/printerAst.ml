...
(* transforme une affectable en chaîne de caractère *)
...
struct
  ...
  | AccesChamp (a,n) -> (string_of_affectable a) ^ "." ^ n
  ...
  let rec string_of_instruction i =
    ...
    | AssignAdd (a,e) -> (string_of_affectable a) ^ " += " ^ (string_of_expression e) ^ "\n"
     |DeclTypeNomme (n,t) -> "typedef " ^ n ^ " = " ^ (string_of_type t) 
...
(*
(* Conversion des definitions des types nommes *)
  let string_of_typenomme (Fonction(t,n,lp,li)) = (string_of_type t)^" "^n^" ("^((List.fold_right (fun (t,n) tq -> (string_of_type t)^" "^n^" "^tq) lp ""))^") = \n"^
                                        ((List.fold_right (fun i tq -> (string_of_instruction i)^tq) li ""))^"\n"*)
