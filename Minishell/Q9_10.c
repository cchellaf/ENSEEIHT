...
void rediriger_sortie(char *sortie) {
   if (sortie !=NULL) {
      int desc_fich = open(sortie, O_WRONLY | O_TRUNC | O_CREAT, 0640); 
      if (desc_fich < 0) {
...
void rediriger_entree(char *entree) {
   if (entree !=NULL) {
   int desc_fich = open(entree, O_RDONLY); 
      if (desc_fich < 0) {
         perror("Erreur d'ouverture"); 
         exit(1); 
      }
...
void commandes(struct cmdline *commande) {
   ...
   if (retour < 0) {   
    ...
    /* fils */ 
       ...
       if (!(commande->in == NULL) ) {
          rediriger_entree(commande->in); 
       } 
       if (!(commande->out == NULL) ) {
          rediriger_sortie(commande->out);
       }
...
void tubes(struct cmdline *commande) {
   int nb_cmd = 0;     //Nombre de commandes
   //int i = 0;          //Indice de commande 
   while (commande->seq[nb_cmd] != NULL) {
      nb_cmd ++;
   }
   ...
   while (i < nb_cmd) {
      ...
      /* le fils */
         ...
         if ( execvp(commande->seq[i][0], commande->seq[i]) < 0) {
