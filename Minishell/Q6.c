...
void commandes(struct cmdline *commande) {
   ...
   if (retour < 0) {   
    ...
    /* fils */ 
       ...
       //On masque le signal SIGTSTP pour tous les fils, afin que le père en a seul accès

       sigset_t ens_signaux; 
       sigemptyset(&ens_signaux);

       /* ajouter SIGTSTP à ens_signaux*/
       sigaddset(&ens_signaux, SIGTSTP);

       /* masquer le signal SIGTSTP */
...
void handler_ctrlC(int sig) {
   printf("Le fils est tué par un signal \n"); 
}
